...
import com.example.job.service.EmailServices;
import com.example.job.service.ErrorHandling;
...
public class UsersManagement {
    ...
    @Autowired
    public UsersManagement(UsersRepository usersRepository, PasswordEncoder passwordEncoder,
            RoleRepository roleRepository, EmailServices emailServices, ResetRepository resetRepository) {
    ...
    public String registration(ErrorHandling err, RegisterDTO regis, Model model,
        ...
        try {
            ...
            String body = "Halo " + regis.getFullname()
                    + ", Selamat akun anda berhasil dibuat, silahkan untuk melakukan login ke link berikut http://localhost:8080/user/login";

            // TODO: PART TO SEND A MAIL AFTER SUCCESS REGISTRATION
            emailServices.sendMail(regis.getEmail(), "Account Registered", body);

            redirectAttributes.addFlashAttribute("successMessage", "Successfully create an account");
    ...
    public String authenticate(LoginDTO login, ErrorHandling err, RedirectAttributes redirectAttributes) {
    ...
    public String saveChange(RedirectAttributes redirectAttributes,
        ...
        try {
            ...
            String body = "Halo " + user.getPerson().getFullname()
                    + ", Selamat password anda berhasil diubah, ini adalah password anda yang baru: "
                    + pass.getNewPassword();

            // TODO: PART TO SEND A MAIL AFTER SUCCESS REGISTRATION
            emailServices.sendMail(user.getEmail(), "Successfully Change Password", body);

            redirectAttributes.addFlashAttribute("successMessage", "Successfully change a password");
            return "redirect:/user/dashboard";
        } catch (Exception e) {
            redirectAttributes.addFlashAttribute("errorMessage", e.getMessage());
    ...
    public String forgotValidate(LoginDTO validate, RedirectAttributes redirectAttributes, ErrorHandling err) {
        try {
            ...
            reset.setUserId(reset.getId());
            reset.setToken(token);
            reset.setExipireDate(new Date(System.currentTimeMillis() + 30 * 60 * 1000));
            resetRepository.save(reset);

            String linkToken = "http://localhost:8080/user/forgot-password/reset?token=" + token;
            String body = "Halo " + user.getPerson().getFullname()
            ...
            redirectAttributes.addFlashAttribute("successMessage", "Silahkan cek email anda");
    ...
    public String reset(@PathVariable("token") String token, Model model, RedirectAttributes redirectAttributes) {
        ResetPassword reset = resetRepository.findByToken(token);

        if (reset == null || reset.getExipireDate().before(new Date()) || reset.isUsed()) {
